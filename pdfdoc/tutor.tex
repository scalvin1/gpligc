...
We have to find a logged position (by left-clicking in the barogram strip or using \keys{F1}--\keys{F4} to move the cursor),
...
Pressing \keys{F8} or \keys{F9} will open a list of thermals (Fig.~\ref{thermals}) or a list of glide distances (Fig.~\ref{glide}).
...
\subsubsection{Winch}
Using the cursor (move with \keys{F1}--\keys{F4} or with the mouse), the winch take-off can be found between 13:35:26 and 13:36:05. To analyse the winch launch mark the first point with \keys{F5}, the second with \keys{F6}. Statistics is then calculated with \keys{F7}.
...
The pictures are not included in the example, but the photo-locator option would look like Fig.~\ref{pic88}, showing the positions of the pictures. Btw. Fig.~\ref{pic88} was created using the postscript output via the keys \keys{o} and \keys{i}.
...
13:36:29--13:39:35 corresponds to the first lift at the ridge up to ca.~1500~m. Selecting this range (\keys{F5}/\keys{F6}) and pressing \keys{F7} will analyse the wind. The opened gnuplot window, shows how the groundspeed depends on the heading (if airspeed is recorded -- as e.g. in some Zander loggers -- the difference groundspeed-airspeed is used). A sinus function is fitted to the data, which gives the direction and speed of the wind. In this case its ca.~40km/h from 137$^\circ$. The direction is typical for this part of the Inn valley in foehn situations.
